...
router.post("/generate", async (req, res) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const expiresAt =
      mode === "time" ? new Date(Date.now() + 2 * 60 * 1000) : null;
...
router.post("/use-token", async (req, res) => {
  ...
  try {
    const result = await db.query("SELECT * FROM tokens WHERE token = $1", [
      token,
    ]);
    ...
    if (
      tokenData.usage_limit &&
      tokenData.usage_count >= tokenData.usage_limit
    ) {
      return res
        .status(410)
